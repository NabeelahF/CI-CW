...
class MasterEnergyPipeline:
    ...
    # ---------------------------------------------------------
    # FIXED: CALCULATE STATS (SOLAR-AWARE)
    # ---------------------------------------------------------
    ...
    def run_single_objective(self, solar_gen):
        ...
        self.print_explicit_schedule(best_sol, solar_gen,"Single-Objective RL (Profit Focused)")
        return best_sol

    # ---------------------------------------------------------
    # PHASE 4: MULTI OBJECTIVE (RL-ADAPTIVE + SMART SELECTION)
    # ---------------------------------------------------------
    def run_multi_objective(self, solar_gen):
        ...
        # Smart Filter: Find profitable strategies that don't destroy the battery
        wear_limit = 40.0 # Max 15 kWh wear allowed

        valid_indices = [
            i for i, w in enumerate(all_wear)
            if w <= wear_limit and all_costs[i] < -0.01  # <--- NEW RULE: Must make at least 1 penny
        ]
    ...
    # ---------------------------------------------------------
    # FIXED: HEAD-TO-HEAD COMPARISON (SOLAR-AWARE)
    # ---------------------------------------------------------
